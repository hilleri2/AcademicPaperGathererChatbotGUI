# scholar_tool_gui_refactored_fixed.py
import sys
import os
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Pre-init placeholders so changeEvent doesn't explode if it fires early
    ...
    def init_ui(self):
        ...
        app_size = app.font().pointSize() if app is not None else 12
        if app_size <= 0:
            app_size = 12
        ...
        # Font size slider with display label (controls GLOBAL font)
        font_layout = QHBoxLayout()
        font_label = QLabel("Font Size:")
        self.font_size_display = QLabel(str(app_size))
        self.font_slider = QSlider(Qt.Horizontal)
        self.font_slider.setRange(8, 32)
        self.font_slider.setValue(app_size)
        ...
        # Apply current app font AFTER widgets exist (avoids early FontChange before slider is set)
        if app is not None:
    ...
    # Keep slider/label in sync when GLOBAL font changes elsewhere
    def changeEvent(self, event):
        if event.type() == QEvent.FontChange:
            # Guard: controls may not be ready during early init
            if self.font_slider is not None and self.font_size_display is not None:
                new_size = self.font().pointSize()
                if new_size > 0:
                    try:
                        self.font_slider.blockSignals(True)
                        self.font_slider.setValue(new_size)
                        self.font_size_display.setText(str(new_size))
                    finally:
                        self.font_slider.blockSignals(False)
        super().changeEvent(event)

    # Slider drives the GLOBAL font so hub/other windows update too
    def change_font_size(self, value):
        self.current_font_size = value
        if self.font_size_display is not None:
            self.font_size_display.setText(str(value))
        f = QFont()
        f.setPointSize(value)
        QApplication.instance().setFont(f)  # global update
        # Optional: immediate local update (usually redundant)
        self.setFont(f)
...
def main():
    ...
    # If launched standalone, ensure a sensible global default
